...
import { INITIAL_CHARGES, INITIAL_MONTHLY_ROOM_INVOICE, INITIAL_RENTAL_ROOM, INITIAL_ROOM_CODE } from '@/initials/RentalRoom.initial';
import { ChargesType, MonthlyRoomInvoiceType, RentalRoomType, RoomCodeType } from '@/types/RentalRoom.type';
...
export const MonthlyRoomInvoiceDetails = (props: MonthlyRoomInvoiceDetailsProps) => {
  ...
  const [roomCodeData, setRoomCodeData] = useState<RoomCodeType>(INITIAL_ROOM_CODE);
  ...
  useEffect(() => {
    const fetchData = async () => {
      try {
        ...
        const roomCodeData = await roomCodeService.get(monitoringRentalData.room_code ?? '');
        
        const [roomData, chargesData] = await Promise.all([
          rentalRoomService.get(roomCodeData.rental_room ?? ''),
          chargesService.getMany({ rental_room: roomCodeData.rental_room, first_only: true }),
        ]);
  ...
  const cancelOnClick = async () => {
    router.push(`/monitoring-rentals/${props.monitoringRentalId}/monthly-room-invoices`);
  };
  ...
  const electricityUsage = (data.new_kWh_reading ?? 0) - (data.old_kWh_reading ?? 0);
  const waterUsage = (data.new_m3_reading ?? 0) - (data.old_m3_reading ?? 0);
  const electricityCost = electricityUsage * (chargesData.electricity_charge ?? 0);
  ...
  return (
    <div className='max-w-4xl mx-auto p-6 bg-white shadow-lg rounded-lg border border-gray-200'>
      ...
      <div className='mb-6'>
        <p><strong>Tên phòng trọ:</strong> {roomData.name || 'N/A'}</p>
        <p><strong>Mã phòng:</strong> {roomCodeData.value || 'N/A'}</p>
        <p><strong>Ngày tạo:</strong> {formatDate(data.created_at, 'dmy')}</p>
      </div>
        ...
        <tbody>
          <tr>
            ...
            <td className='border p-2'>
              {data.continue_renting ? `${formatCurrency(chargesData.room_charge)}/tháng` : '-'}</td>
            <td className='border p-2'>
          ...
          <tr>
            ...
            <td className='border p-2'>{formatCurrency(chargesData.electricity_charge)}/kWh</td>
          ...
          <tr>
            ...
            <td className='border p-2'>{formatCurrency(chargesData.water_charge)}/m<sup>3</sup></td>
          ...
          <tr>
            ...
            <td className='border p-2'>-</td>
            <td className='border p-2'>
              {chargesData.wifi_charge !== -1 ? `${formatCurrency(chargesData.wifi_charge)}/tháng` : '-'}
            </td>
            <td className='border p-2'>
          ...
          <tr>
            ...
            <td className='border p-2'>-</td>
            <td className='border p-2'>-</td>
            <td className='border p-2'>{formatCurrency(chargesData.rubbish_charge)}/tháng</td>
          ...
          <tr>
            <td className='border p-2 italic font-bold' colSpan={5}>Tổng số tiền phải trả</td>
            <td className='border p-2 italic font-bold'>{formatCurrency(totalDue)}</td>
          </tr>
          <tr>
            <td className='border p-2 italic font-bold' colSpan={5}>Tổng số tiền đã trả</td>
            <td className='border p-2 italic font-bold'>{formatCurrency(paid)}</td>
          </tr>
        </tbody>
      </table>

      <div className='mb-6'>
        <p><strong>Ngày cập nhật:</strong> {formatDate(data.updated_at, 'dmy')}</p>
        <p><strong>Trạng thái:</strong> {data.is_settled ? 'Đã kết toán' : 'Chưa kết toán'}</p>
      </div>
