...
export const RentalRoomsList = () => {
  ...
  const [query, setQuery] = useState<RentalRoomQueryType>(
    INITIAL_RENTAL_ROOM_QUERY
  );
  ...
  useEffect(() => {
    const fetchData = async () => {
      try {
        ...
        const [data, provinceData, districtData, communeData] =
          await Promise.all([
            rentalRoomService.getMany({
              manager_is_null: false,
              _renter: myIdRef.current,
            }),
      ...
      try {
        ...
        if (viewMode === "all") {
          ...
          const data = await rentalRoomService.listByIds({
            _recommendation_list: recommendationList,
            _renter: myIdRef.current,
          });
          originalDataRef.current = [...data];
          setData([...originalDataRef.current]);
  ...
  const detailsFunction = useCallback(
    async (id: string) => {
  ...
  const filterOnClick = async () => {
    try {
      ...
      if (
        ...
        const districts = await districtService.getMany({
          province: query._province,
        });

        const communesArray = await Promise.all(
          districts.map((district) =>
            communeService.getMany({ district: district.id })
          )
        );
        ...
        const communes = await communeService.getMany({
          district: query._district,
        });
        const dataArray = await Promise.all(
          communes.map((commune) =>
            rentalRoomService.getMany({
  ...
  const handleProvinceQueryChange = (
    e: React.ChangeEvent<HTMLSelectElement>
  ) => {
    setQuery({ ...query, _province: e.target.value });
    if (e.target.value === "") {
      setDistrictOptions(
        mapOptions(originalDistrictDataRef.current, ["name"], "id")
      );
    } else {
  ...
  const handleDistrictQueryChange = (
    e: React.ChangeEvent<HTMLSelectElement>
  ) => {
    setQuery({ ...query, _district: e.target.value });
    if (e.target.value === "") {
      setCommuneOptions(
        mapOptions(originalCommuneDataRef.current, ["name"], "id")
      );
    } else {
  ...
  const handleCommuneQueryChange = (
    e: React.ChangeEvent<HTMLSelectElement>
  ) => {
    setQuery({ ...query, commune: e.target.value });
  };

  const handleRoomChargeQueryChange = (
    e: React.ChangeEvent<HTMLSelectElement>
  ) => {
    setQuery({ ...query, _room_charge_range: e.target.value });
  };

  const handleEmptyModeQueryChange = (
    e: React.ChangeEvent<HTMLSelectElement>
  ) => {
      ...
      <Taskbar>
        ...
        <div className="ml-[70px] flex items-center space-x-4">
          <Label htmlFor="view-mode">
            <p className="font-semibold">Chế độ xem:</p>
          </Label>
          <Select
            id="view-mode"
            options={[
